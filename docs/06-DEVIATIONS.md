...
| `scripts/Test-Workspace.ps1` | METAPROMPT specifikoval 8 kontrol. Follow-up jich vyžadoval 10 dalších okruhů. | Skript má nyní **15 kontrol** (14 původních + **integrita `.env`** přidaná ve verzi 1.0.2): adresáře, soubory, UTF-8 BOM, PSScriptAnalyzer, secrets, Node, git, CRLF, **LF u .md/.json/.toml**, **komentářová nápověda**, **odkazy v README**, **HTML landing page**, **fáze METAPROMPT**, **YAML frontmatter promptů**, **integrita .env**. | Hotovo. Nové kontroly mají `-Fix` tam, kde je oprava bezpečná (LF koncovky). |
