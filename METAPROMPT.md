...
\- \*\*DeepSeek API\*\*: klíč dostupný přes `$env:DEEPSEEK\_API\_KEY` nebo `.env`
...
1\. Ověř, že jsi na Windows (`$IsWindows` nebo `$env:OS -eq "Windows\_NT"`)
...
\- Odstranění `node\_modules` z trackingu
